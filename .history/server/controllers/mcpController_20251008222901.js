...
const tripHandler = async (req,res) => {
    ...
    const getWeatherTool = tool(
        async (args) => {
            const { city, travelDate } = args; // now safe
    ...
    }).bindTools([getWeatherTool,getRouteTool]); // Bind your tools to the model
...
const getWeather=async(res)=>{
       if(!isWeather)return res.json([]);
       return res.json(weather);
}

const getRoute=async(res)=>{
    if(!isRoute)return res.json([]);
    return res.json(route);
}
module.exports = { tripHandler ,getWeather ,getRoute }