...
const {getItinerary} =require("./itineraryController");
const { z } = require("zod");
...
const tripHandler = async (req, res) => {
  ...
  const getRouteTool = tool(
    ...
    },
    {
      name: "get_route",
      description:
        "Useful for when you have to get the route details from a city to another city",
      schema: z.object({
   ...
   const getItineraryTool = tool(
     async (args) => {
      ...
      end_date=endDate;
      return await getItinerary(destination,startDate,endDate);
    },
    {
      name: "get_itinerary",
      description:
